...
// т.к. √(1+8*S) всегда будет нечетным
// (т.к. 8*S - четное, 1+8*S - нечетное, а четное * четное не может быть нечетным)
// то (-1 + √(1+8*S)) будет четным, т.е. (-1 + √(1+8*S))/2 всегда целое число.
// Тогда только нужно, чтобы волнялось условие √(1+8*S) = целое число
